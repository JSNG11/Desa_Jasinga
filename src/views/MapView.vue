...
export default {
  ...
  data() {
    return {
      ...
      enableTooltip: true,
      zoom: 15,
      center: [-6.485863275706096, 106.45230182111699],
      layerData: [],
      geojson: null,
      fillColor: "#0CF9E0",
      tileProviders: tileProviders,
      marker: latLng(-6.485863275706096, 106.45230182111699),
      timeout: undefined,
    };
  },
  mounted() {
  ...
  async created() {
    this.loading = true;
    const response = await fetch("/Penggunaan_Lahan_FeaturesToJ.json");
    const data = await response.json();
    this.geojson = data;
    this.loading = false;
  },
